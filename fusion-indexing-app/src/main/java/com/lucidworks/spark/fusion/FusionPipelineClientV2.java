...
public class FusionPipelineClientV2 {

  private static final Logger log = LoggerFactory.getLogger(FusionPipelineClientV2.class);
  ...
  public FusionPipelineClientV2(String fusionHostAndPortList) throws MalformedURLException {
    this(fusionHostAndPortList, null, null, null, null, null);
  }

  public FusionPipelineClientV2(String fusionHostAndPortList, String fusionUser, String fusionPass, String fusionRealm) throws MalformedURLException {
    this(fusionHostAndPortList, fusionUser, fusionPass, fusionRealm, null, null);
  }

  public FusionPipelineClientV2(String fusionHostAndPortList, String fusionUser, String fusionPass, String fusionRealm, String trustedRealmHeader, String[] trustedRealmGroups) throws MalformedURLException {
  ...
  public void postInputStreamToPipeline(String pipelinePath, IndexDirectoryToFusion.PortableInputStream inputStream) throws Exception {
    ...
    if (mutable.size() > 1) {
      ...
      while (!mutable.isEmpty()) {
        ...
        if (log.isDebugEnabled())
          log.debug("POSTing stream of input docs to " + hostAndPort + pipelinePath + " as request " + requestId);

        Exception retryAfterException =
            postStreamToPipelineWithRetry(hostAndPort, pipelinePath, inputStream, mutable, lastExc, requestId);
      ...
      String hostAndPort = getLbServer(mutable);
      if (log.isDebugEnabled())
        log.debug("POSTing batch of docs to " + hostAndPort + pipelinePath + " as request " + requestId);

      Exception exc = postStreamToPipelineWithRetry(hostAndPort, pipelinePath, inputStream, mutable, null, requestId);
      if (exc != null)
        throw exc;
    }
  }
  ...
  protected Exception postStreamToPipelineWithRetry(String hostAndPort,
                                                  String pipelinePath,
                                                    IndexDirectoryToFusion.PortableInputStream inputStream,
                                                  ArrayList<String> mutable,
                                                  Exception lastExc,
                                                  int requestId)
      throws Exception {
    ...
    try {
      postInputStreamRequestToPipeline(hostAndPort, pipelinePath, inputStream.open(), requestId);
      if (lastExc != null)
  ...
  public void postInputStreamRequestToPipeline(String hostAndPort, String pipelinePath, InputStream inputStream, int requestId) throws Exception {
    ...
    log.info("Sending input stream to url {}", postUrl);
    InputStreamEntity et = new InputStreamEntity(inputStream, ContentType.APPLICATION_OCTET_STREAM);
